...
如果纪要标题出现“基于转写原文的保守整理”，说明系统发现大模型原始纪要证据不足，已经自动换成更贴近转写原文的版本。这个版本可能不够漂亮，但更适合先复核、再对外发送。系统也会在保存新生成待办前删除完全找不到转写依据的模型待办；如果全部模型待办都缺证据，会保留一条带原文片段的“按转写原文复核待办”，提醒你先看原文，而不是直接发起督办。
