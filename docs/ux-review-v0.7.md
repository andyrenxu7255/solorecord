...
- Web 详情增加音频分段列表。
- Web 详情增加状态说明、说话人统计、按说话人/来源分段筛选转写、授权音频加载播放。
- Web 筛选后保存转写会保留隐藏段落，避免误删。
- Android 详情页把纪要、待办和说话人统计提前，便于先校对会后成果。
...
- Web 详情新增状态说明、说话人统计、转写筛选、音频分段播放入口和可编辑待办。
- 服务端新增待办更新接口，写入后同步、导出和外部 API 都能读取。
- Web 录音上传失败时保留当前页面缓存并提供重试入口。
- Android 详情页增强会后校对顺序和说话人统计。
