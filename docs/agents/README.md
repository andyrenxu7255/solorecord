...
- 多源同录：1-8 个录音源通过同一 `join_code` 加入同一会议。重复加入时，同一用户、同一设备名和同一录音源名称应复用原 `source_id`；同一用户显式换录音源名称时可作为另一台设备。`/api/mobile/meetings/discover` 和 `/api/web/meetings/discover` 只返回可加入会议元数据，不能返回转写、纪要、音频、待办或成员详情；用户仍需 join 后才可读会议详情。证据键是 `(source_id, source_segment_no)`；不要只用本地分段号判断覆盖或替换。待办、纪要和知识入库风险判断如果已有 `segment_id`，必须按转写行精确匹配冲突/多数源 flags；只有缺少 `segment_id` 的旧式或模拟证据才退回来源分段键。最终处理会合并重复多源片段并标记 `multi_source_merged`；设备错峰起录但来源分段相邻、时间差和文本相似度满足保守阈值时，合并片段还会标记 `multi_source_time_aligned`。至少两个来源一致且多于附近冲突来源时，主结果标记 `multi_source_majority`；少数冲突片段仍标记 `multi_source_conflict`。
...
12. 质量联调：检查 `qualityReport` 中的 `speaker_review_count`、`speaker_evidence_weak_count`、`speakerEvidence`、`summaryEvidence.supportedClaims`、`summaryEvidence.contradictedClaims`、`summaryEvidence.unsupportedClaims`、`actionEvidence`、`multiSourceConflicts`、`generic_owner_count`、`unsupported_action_count`、`summary_contradiction_count`、`summary_unqualified_conflict_count`、`action_evidence_coverage`、`timeline_repaired_count` 和 `scenario_counts`；真实会议不要只看纪要是否“像样”，还要看发言人、纪要和待办是否能从转写原文中找到证据。先跑 `quality_probe --postprocess` 看本地规则模拟，再按需跑 `--run-llm`；两种模拟结果里的 `delta.improved_metrics`、`delta.regressed_metrics`、`delta.speaker_names_added`、`delta.action_owner_changes`、`delta.suggested_owner_changes` 可以快速判断混合段、发言人和待办负责人是否真的变好。`action_owner_changes` 表示模拟后已能直接修正 owner，`suggested_owner_changes` 表示仍需人工应用的建议。`summaryEvidence.supportedClaims[].status=majority` 可作为多数源主证据但要保留抽查提示，`status=conflict` 只由冲突源支撑，纪要必须保留待确认语气，`status=contradiction` 表示纪要与同主题转写证据相反，必须阻断知识入库；如果 LLM 把冲突事实写成确定结论，或把“未完成/不要发”等原文写成“已完成/已发送”，服务端应降级为保守整理。若会议纪要标题为“基于转写原文的保守整理”，表示 LLM 原始纪要证据不足、反向证据或冲突语气不合格，服务端已自动降级为证据优先版本；降级后的冲突行仍应显示“多源冲突待确认”，并保持 `summary_unqualified_conflict_count=0`。
...
- `qualityReport.metrics.speaker_over_split_count` 或 `too_many_speakers` issue 表示发言人标签异常偏多。真实会议验收时，如果实际参会人很少但出现十几个到几十个 speaker，必须优先校对/合并人物标签，不要直接把这些标签当成知识图谱里的独立人员。
...
- `ontologyGraph` 是持久化本体图谱，不是详情页临时拼接结果。实体类型固定为 `person`、`place`、`time`、`matter`、`action`，关系类型优先使用 `responsible_for`、`due_at`、`located_at`、`scheduled_at`、`discussed`、`related_to`、`depends_on`、`mentioned`。规则兜底也会用“依赖、等待、需要、先”等上下文建立 `action -> action` 的 `depends_on` 边，维护 Agent 验收时要检查这类边是否带有转写/待办证据。`processing_jobs.type=knowledge_graph` 表示独立抽槽任务；会议最终处理成功后会自动落库，也可通过 `/api/web/meetings/{meetingId}/ontology/extract` 单独重建。外部知识 Agent 读取 `/api/external/meetings/{meetingId}/ontology`，但入库证据仍要回看 `/transcript?include_history=true`。
...
- `qualityReport.metrics.source_segment_coverage` 和 `qualityReport.sourceCoverage.weakSegments` 应按 `(source_id, source_segment_no)` 反映每个上传音频分段是否被最终转写有效覆盖。`mock_asr`、`empty_asr`、`missing_audio` 和 `source_coverage_gap` 是复核占位，不算有效覆盖；最终 `/finish` 可补出 `source_coverage_gap` + `missing_audio` 的“系统复核”转写行，让缺失音频分段可定位但仍阻断入库。复核占位行只用于定位和阻断入库，不得作为会议事实参与 `speakerEvidence`、候选人名、纪要证据、待办证据、说话人统计或 `knowledgeGraph` 主题。`source_segment_coverage_weak` 是知识入库阻塞项，外部知识 Agent 不得把该会议视为完整证据。`multi_source_conflict_count` 大于 0 时也应保留人工复核状态。
...
- 多源互补只能复制原始转写中存在、且与当前合并段不冲突的短语；`multi_source_complemented` 表示证据融合，不表示 LLM 生成了新事实。若至少两个来源一致且多于附近冲突来源，可把一致合并段标记为 `multi_source_majority`，同时保留少数冲突源供人工回听；历史合并行如果已有 `multi_source_merged` 和 `multi_source_count:*` 但缺少 `multi_source_majority`，服务端会在 API/质量报告返回层派生多数源标记，不需要运维 Agent 重处理音频或改写转写历史；没有多数时必须优先标记 `multi_source_conflict`。
...
- 如果 LLM 输出 owner 为“负责人、相关负责人、主持人、前端开发、某某负责人”等泛化角色，`llm_adapters._normalize_owner()` 应先降级为 `待确认`，再由服务端基于转写证据尝试补回具体人或明确团队。明确组织 owner（如销售、法务、前端）可以保留。
...
- Multi-source recording: 1-8 sources join the same meeting through `join_code`. Repeated joins should reuse the existing `source_id` when the same user, device name, and source label match; the same user may register another device by changing the source label. `/api/mobile/meetings/discover` and `/api/web/meetings/discover` return joinable meeting metadata only; they must not expose transcripts, summaries, audio, action items, or member details, and users still need to join before reading meeting details. The evidence key is `(source_id, source_segment_no)`, not `source_segment_no` alone. For action, summary, and knowledge-ingestion risk checks, `segment_id` is the precise pointer when present; fall back to the source-segment key only for ID-less legacy/probe evidence. Final processing merges duplicate multi-source rows with `multi_source_merged`; if devices start at different times but source-local segment numbers, start-time delta, and text similarity pass conservative checks, merged rows also carry `multi_source_time_aligned`. If at least two sources agree and outnumber nearby conflicting sources, the primary row gets `multi_source_majority`; minority divergent rows are still flagged with `multi_source_conflict`.
...
- `qualityReport.metrics.speaker_over_split_count` or a `too_many_speakers` issue means speaker labels are unusually high. If a real meeting had only a few attendees but the transcript has dozens of speakers, calibrate and merge labels before treating them as separate people in downstream knowledge graphs.
...
- `ontologyGraph` is the persisted ontology graph, not a temporary detail-page projection. Entity types are limited to `person`, `place`, `time`, `matter`, and `action`; relation types should prefer `responsible_for`, `due_at`, `located_at`, `scheduled_at`, `discussed`, `related_to`, `depends_on`, and `mentioned`. The rule fallback also creates `action -> action` `depends_on` edges from context markers such as "depends on", "wait for", "need", or "first"; maintenance agents should verify those edges include transcript/action evidence. `processing_jobs.type=knowledge_graph` is the standalone slot-extraction job. Final meeting processing stores it automatically, and clients can rebuild only the graph through `/api/web/meetings/{meetingId}/ontology/extract`. External knowledge agents read `/api/external/meetings/{meetingId}/ontology`, but ingestion evidence must still be checked against `/transcript?include_history=true`.
- `qualityReport.metrics.action_evidence_coverage` should show whether action items are grounded in transcript evidence. If `unsupported_action_count` is above zero, the Web UI should warn reviewers before the action list is shared. Newly generated LLM actions with `unsupported` evidence should be removed before saving. If every generated action is unsupported, keep one transcript-referenced review action and never auto-send it. If no LLM is configured but ASR produced real transcript text, do not create a synthetic "check transcript result" action; keep that reminder only for `mock_asr`, `empty_asr`, or `missing_audio` placeholder rows. An action already classified as `unsupported` should not also appear in `weakActionOwners`; downstream agents should see one clear missing-evidence risk, not two overlapping risks.
- `qualityReport.metrics.source_segment_coverage` and `qualityReport.sourceCoverage.weakSegments` should show whether each uploaded audio segment is substantively covered by the final transcript, keyed by `(source_id, source_segment_no)`. `mock_asr`, `empty_asr`, `missing_audio`, and `source_coverage_gap` are review placeholders, not effective coverage; final `/finish` may add a `source_coverage_gap` + `missing_audio` "system review" transcript row so the missing audio segment remains locatable while ingestion stays blocked. Placeholder rows are only navigation and ingestion-blocking evidence; they must not be treated as meeting facts for `speakerEvidence`, candidate people, summary evidence, action evidence, speaker counts, or `knowledgeGraph` topics. `source_segment_coverage_weak` blocks knowledge ingestion. If `multi_source_conflict_count` is above zero, downstream agents should keep the meeting in human-review status.
...
- `qualityReport.actionEvidence` and `qualityReport.summaryEvidence` should treat `segment_id` as the precise evidence pointer when it is present. Conflict or majority status must come from that exact transcript row; use `(source_id, source_segment_no)` only as a fallback for ID-less legacy/probe evidence. This prevents a conflict in one topic from contaminating a separate safe action in the same source segment.
- Conservative transcript-grounded summaries must still label conflict-backed lines as pending confirmation. When fallback summary or role-note text references `multi_source_conflict` rows, keep explicit review wording so `summary_unqualified_conflict_count` remains 0.
...
- If the LLM outputs a generic action owner such as "owner", "related owner", "host", "frontend developer", or a label ending with "owner", `llm_adapters._normalize_owner()` should first downgrade it to `待确认`; the server may then restore a concrete person or explicit team only from transcript evidence. Clear organizational owners such as sales, legal, or frontend may be preserved.
...
- External `actionItems` entries directly include `evidenceStatus`,
  ...
  auto-send it. `evidenceStatus=contradiction` means the action reversed
  same-topic transcript blockers such as "do not send yet", "pause", "cannot",
  or "cancel" into an executable task; reminder agents must also block it and
  wait for a rewritten or removed action. Use `qualityReport.actionEvidence`
  for complete evidence audits.
- External `knowledgeReadiness` is the ingestion gate summary. `status=hold`
  ...
  The Web meeting detail page mirrors this as a knowledge ingestion review
  block for human acceptance. Weak coverage entries may not have a transcript
  row yet; replay the audio or trigger re-transcription instead of treating the
  review row as transcript evidence.
