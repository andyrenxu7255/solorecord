...
- `qualityReport.actionEvidence` 会逐条列出待办证据状态：`supported` 表示可找到转写依据，`majority` 表示多数录音源已确认但仍建议抽查，`system_review` 表示系统复核入口而非会议待办，`weak_owner` 表示任务有依据但负责人证据弱，`unsupported` 表示缺转写依据，`conflict` 表示只由冲突片段支撑，`contradiction` 表示待办把转写里的“先不要、暂缓、不能、取消”等反向证据写成了执行动作。外部 API 的 `actionItems` 也会直接带 `evidenceStatus`、`evidence`、`suggestedOwner`、`actionKind`、`autoActionable`、`reminderSafe`、`knowledgeSafe` 和 `requiresReview`，方便督办 Agent 不解析完整质量报告也能判断是否可自动提醒。多数源只增强任务文本证据；如果负责人仍是 `待确认`、代词、时间短语或泛化角色，状态必须优先保持 `weak_owner`、`knowledgeSafe=false`，不能自动督办。`system_review` 条目必须保持 `autoActionable=false`、`reminderSafe=false`，不得同步成提醒。若系统能找到更有证据的负责人，会同时给出 `suggested_owner` 和引用片段，Web 待办区会显示“建议负责人/应用建议”。证据引用带 `segment_id`、`source_id` 和 `source_segment_no`，Web 可用“定位转写”跳到对应原文。复制待办给 IM 时仍只复制待办正文，并跳过系统复核提醒。
- 运维验收时注意风险去重：`unsupported` 表示待办任务本身缺转写证据，这类条目不应再出现在 `weakActionOwners` 里；否则前端会重复提示，外部督办 Agent 也可能误判为两个独立风险。建议负责人可来自粗分段里的点名句，例如当前行仍是主持人或 `发言人 1`，但文本包含“翼天你说自动测试”“李波后面看登录界面”，系统可以给出被点名人的 `suggested_owner`。同时要确认“舞台音响、自动测试、测试覆盖、数据源”等议题词没有被当作 owner。
- 验收多源冲突时注意粒度：`source_id/source_segment_no` 用于音频覆盖、分段重传和回听定位；如果待办或纪要证据已经带 `segment_id`，冲突/多数源状态必须按该转写行判断。同一个 5 分钟来源分段里可以同时有一个冲突议题和一个安全待办，安全待办不应显示“多源冲突待核对”。
...
- 会议详情页采用轻量概要先打开，优先显示人物校对、会议纪要和待办，再补齐转写和质量证据。本体抽槽、服务端图谱构建和独立图谱页面已移除；知识平台需要关系分析时，应基于外部转写接口、质量报告、待办证据和 ES/OpenSearch 索引生成。
...
第一条只读当前持久化结果；`--postprocess` 只跑服务器本地后处理模拟，并用模拟后的分段重新归一化待办负责人；`--run-llm` 会调用已配置的大模型做模拟重分段和纪要探测。三种模式都不会写回数据库。上线前重点看 `qualityReport.status`、候选人名、`speaker_review_count`、`timeline_repaired_count`、`generic_owner_count` 和待办负责人是否可接受；如果使用 `--postprocess` 或 `--run-llm`，还要看 `delta.improved_metrics`、`delta.regressed_metrics`、`delta.speaker_names_added`、`delta.action_owner_changes` 和 `delta.suggested_owner_changes`，确认混合段减少、发言人更完整、待办负责人已被直接修正，或只剩需要人工应用的建议。
...
- Empty remote STT text is saved with an `empty_asr` placeholder transcript so the meeting remains visible, exportable, and reviewable. Placeholder rows are not counted as effective source-segment coverage and are excluded from speaker review, summary/action evidence, and knowledge-graph topics; they keep the review target visible while `source_segment_coverage_weak` blocks automatic ingestion.
...
- If the LLM only splits a long native-ASR speaker row and keeps the same `speaker_id`, the final rows keep `asr_speaker`. If the LLM changes the speaker from context, task ownership, or topic continuation, the row stays under `speaker_review` and must be treated as a review hint, not native speaker evidence.
...
- If no LLM is configured but ASR produced real transcript text, SoloRecord saves the conservative transcript-grounded summary without creating a synthetic "check transcript result" action. That reminder action is reserved for placeholder rows such as `mock_asr`, `empty_asr`, or `missing_audio`. System review entries are exposed as `evidenceStatus=system_review`, `actionKind=system_review`, `autoActionable=false`, and `reminderSafe=false`; downstream reminder systems must skip them. Android local records and Markdown/Word/PDF/JSON exports keep the system-review label.
- LLM summaries are not saved blindly. The server first checks whether summary and role-note claims are supported by transcript evidence. If evidence coverage is too low, any generated claim lacks transcript evidence, a multi-source conflict is stated as definite, or completion/negation evidence is reversed, it replaces the result with a conservative transcript-grounded summary and corrects clearly unsupported action owners when stronger transcript evidence exists. Conservative fallback text that references conflict rows should say that the multi-source conflict is pending confirmation; acceptance can verify `summary_unqualified_conflict_count=0`. Seeing the "conservative transcript-grounded summary" title means the safety fallback was used.
...
- If `speaker_alias_conflict_count` is above zero, one display name maps to multiple `speaker_id` values. Web shows this as a same-name/multiple-label warning. Downstream knowledge systems should use the calibrated speaker name while preserving original IDs for traceability.
- The meeting detail page opens with a lightweight overview first, prioritizes people calibration, summary, and action items, then fills in transcript and quality evidence. Server-side graph building, ontology extraction, and the standalone graph page have been removed; knowledge platforms that need relationships should build them from the external transcript API, quality reports, action evidence, and ES/OpenSearch indexing.
...
- If the transcript says "do not send yet", "pause", "cannot", or "cancel", generated action items should preserve review or pause semantics. Executable actions that contradict these blockers are removed before saving summaries; unsupported generated actions are also removed before saving. If all generated actions were unsafe or unsupported, the server keeps one transcript-referenced review fallback action. Acceptance should verify both an unsafe action such as "send customer notice" is removed and a review action such as "confirm whether to send customer notice" remains.
...
All three modes are read-only. Check `qualityReport.status`, candidate people, `speaker_review_count`, timeline repair count, generic owners, action owner evidence, and `summary_contradiction_count`. Any `summary_evidence_contradiction` blocker means the summary reversed transcript evidence and must not be indexed until rewritten from the transcript. Unsupported actions should appear as missing transcript evidence only, not as both unsupported and weak-owner risks. Suggested owners may come from named call-outs in coarse rows, but topic nouns such as stage audio, automated testing, test coverage, and data source must not become owners. In simulation modes, also inspect `delta.improved_metrics`, `delta.regressed_metrics`, `delta.speaker_names_added`, `delta.action_owner_changes`, and `delta.suggested_owner_changes` to confirm mixed turns decrease, speakers become more complete, action owners are directly corrected when evidence is strong, and only uncertain suggestions remain for human review.
