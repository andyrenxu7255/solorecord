...
这是 iOS 的 WKWebView 外壳工程，用于承载服务器端 SoloRecord Web 体验。它默认连接 `https://record.uino.com`，不内置任何模型、LDAP、SSO、Hermes 或 ES 密钥。
...
4. 确认 `Info.plist` 中 `SoloRecordServerURL` 为 `https://record.uino.com`。
...
IPA packaging requires Xcode on macOS plus company signing certificates.
`SoloRecordServerURL` is preconfigured as `https://record.uino.com`; archive the
app, export an Ad Hoc or Enterprise IPA, then publish it from the SoloRecord Web
admin page.
