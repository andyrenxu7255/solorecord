...
- `qualityReport.metrics.source_segment_coverage` and `qualityReport.sourceCoverage.weakSegments` should show whether each uploaded audio segment is substantively covered by the final transcript, keyed by `(source_id, source_segment_no)`. `mock_asr`, `empty_asr`, `missing_audio`, and `source_coverage_gap` are review placeholders, not effective coverage; final `/finish` may add a `source_coverage_gap` + `missing_audio` "system review" transcript row so the missing audio segment remains locatable while ingestion stays blocked. `source_segment_coverage_weak` blocks knowledge ingestion. If `multi_source_conflict_count` is above zero, downstream agents should keep the meeting in human-review status.
