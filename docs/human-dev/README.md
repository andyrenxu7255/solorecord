...
- `qualityReport.actionEvidence` 会为每条待办输出 `supported`、`conflict`、`weak_owner` 或 `unsupported`，并附带相关转写片段。证据片段必须带 `segment_id`、`source_id` 和 `source_segment_no`，便于 Web 的“定位转写”和外部 Agent 追溯原文。`conflict` 表示待办可以找到转写证据，但证据来自 `multi_source_conflict` 片段，外部督办 Agent 必须视为需人工回听，不能自动发送提醒或写入确定知识。若能从任务关键词和责任表述中找到更接近的人，还会输出 `suggested_owner`、`suggested_owner_reason` 和 `suggested_owner_evidence`。`actionItems` 同步带有驼峰命名的 `evidenceStatus`、`evidence`、`suggestedOwner`、`knowledgeSafe` 和 `requiresReview`，方便只消费待办列表的外部督办 Agent 使用。Web 待办区用这些证据展示“有转写依据”“多源冲突待核对”“负责人证据弱”或“缺转写证据”，并提供“应用建议”按钮；复制待办时不包含证据文本。
