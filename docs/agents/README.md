...
- `qualityReport.metrics.speaker_over_split_count` 或 `too_many_speakers` issue 表示发言人标签异常偏多。真实会议验收时，如果实际参会人很少但出现十几个到几十个 speaker，必须优先校对/合并人物标签，不要直接把这些标签当成外部知识里的独立人员。
- 会议详情、外部 API 和默认索引路径不返回 `knowledgeGraph` 或 `ontologyGraph`。本体抽槽、服务端图谱构建和独立图谱页面已移除，不要放回会议详情首屏加载路径。外部知识 Agent 需要关系分析时，应基于 `/api/external/meetings/{meetingId}/transcript?include_history=true`、`qualityReport`、`knowledgeReadiness` 和 `actionItems` 自行生成关系，并保留 `segment_id/source_id/source_segment_no` 原文证据。
...
- “今天下午、下午、周三前、月底前”等日期/截止时间短语只能作为时间事实，不能被当成发言人或负责人。若看到这类词进入 `display_name`、`owner` 或外部知识人员节点，先修后处理规则再联调真实会议；若历史待办 owner 已是时间词，`actionEvidence.status` 应为 `weak_owner`，不能当作可自动督办对象。
...
- 人物试听样本优先使用服务端 `ffmpeg` 裁剪；生产 Docker Compose 默认安装媒体工具，前端仍必须保留浏览器端 5-20 秒样本裁剪兜底。只有服务器包源受限、临时需要轻量镜像时，才用 `INSTALL_MEDIA_TOOLS=false` 关闭服务端转码。PDF 中文渲染如果需要系统字体，单独使用 `INSTALL_CJK_FONTS=true` 构建。
...
- Meeting detail, external APIs, and the default indexing path do not return `knowledgeGraph` or `ontologyGraph`. Server-side graph building, ontology extraction, and the standalone graph page have been removed; do not add them back to the first-load meeting detail path. If downstream knowledge agents need relationship analysis, build it from `/api/external/meetings/{meetingId}/transcript?include_history=true`, `qualityReport`, `knowledgeReadiness`, and `actionItems`, while preserving `segment_id/source_id/source_segment_no` references.
...
- Speaker samples prefer server-side `ffmpeg` clipping. Production Docker Compose installs media tools by default, and the frontend must still keep the browser-side 5-20 second clipping fallback. Disable server-side transcoding with `INSTALL_MEDIA_TOOLS=false` only when the package mirror is constrained and a temporary lightweight image is required. If Chinese PDF rendering needs system fonts, build separately with `INSTALL_CJK_FONTS=true`.
