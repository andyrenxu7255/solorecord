...
- 会议详情、外部 API 和默认索引路径不再返回 `knowledgeGraph` 或 `ontologyGraph`。历史图谱代码可以保留为离线研究能力，但不要把图谱或本体抽槽放回会议详情首屏加载路径。外部知识 Agent 需要关系分析时，应基于 `/api/external/meetings/{meetingId}/transcript?include_history=true`、`qualityReport`、`knowledgeReadiness` 和 `actionItems` 自行生成关系，并保留 `segment_id/source_id/source_segment_no` 原文证据。
...
- Meeting detail, external APIs, and the default indexing path no longer return `knowledgeGraph` or `ontologyGraph`. Legacy graph code may remain for offline experiments, but graph or ontology extraction must not be added back to the first-load meeting detail path. If downstream knowledge agents need relationship analysis, build it from `/api/external/meetings/{meetingId}/transcript?include_history=true`, `qualityReport`, `knowledgeReadiness`, and `actionItems`, while preserving `segment_id/source_id/source_segment_no` references.
